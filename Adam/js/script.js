$(window).scroll(function(){
    $(".number").css("opacity", 1 - $(window).scrollTop() / 2000);
  });



$(window).scroll(function(){
    $(".hero1").css("opacity", 1 - $(window).scrollTop() / 400);
  });
...
TM.amplitude = 13;
TM.frequency = 129;
TM.amplitudeOffset = 1.5;
TM.message = 'adamriguez 2017';
...
;TM.SliceAnim = function(){
	...
	me.drawBuffer = function() {
		var f = TM.IS_RETINA ? 2 : 1,
			...
			colors = ["#000000","#b01cde","#e61d40","#f29613","#ffffd7"];
			buffer = document.createElement('canvas');

		buffer.width = 1000*f;
		buffer.height = TM.LOGO_RECT.height*colors.length*f;
		var tctx = buffer.getContext('2d');
		tctx.font = (TM.LOGO_RECT.height*f)+"px Carrois Gothic SC, sans-serif";
